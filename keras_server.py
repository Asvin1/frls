...
def face_recognition(encoding, database, model, threshold=10):
    min_dist = 99999
    ...
    for email in database.keys():
        ...
        try:
            dist = np.linalg.norm(np.subtract(database[email]['encoding'][0]['embedding'], encoding[0]['embedding']))
            print(dist)
        except:
            dist=99999
...
def add_face():
    ...
    valid_face = False
    try:
        DeepFace.extract_faces('saved_image/new.jpg')
        valid_face = True
    except:
        valid_face = False
    ...
    if valid_face:
        ...
        encoding = DeepFace.represent('saved_image/new.jpg', model_name ="Facenet512")
...
def predict():
    ...
    if flask.request.method == "POST":
        if flask.request.files.get("image"):
            ...
            if valid_face:
                ...
                min_dist, identity, authenticate = face_recognition(
                                                    encoding, user_db, model, threshold=20)
        ...
        if data['authenticate']:
            ...
            flash('Unknown Person!'+data["min_dist"])
